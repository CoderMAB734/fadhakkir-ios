...
const set_cookie = function(cookie_main) {
  document.cookie = `${cookie_main.toString()} expires=Thu, 01 Jan ${new Date().getFullYear() + 1
    } 00:00:00 UTC;`;
};
...
const update_theme = function() {
  ...
  document.querySelector('.three-body').style.setProperty('--uib-color', secondary_color)
  ...
  document.documentElement.style.setProperty('--secondary-blur-color', `${secondary_color}22`);
  document.documentElement.style.setProperty('--primary-color', primary_color);
  document.documentElement.style.setProperty('--primary-blur-color', `${primary_color}cc`);
...
const get_location = async function() {
  ...
  const { latitude, longitude } = cookie_location || (await getCoordinates()).coords;
  set_cookie(`location=${JSON.stringify([latitude, longitude])};`)

  //hiding musjid times if needed
  if (!((latitude < -27.6 && latitude > -27.85) && (longitude > 29.8 && longitude < 30.10))) {
...
const main_code = (latitude, longitude) => {
  ...
  getCity(latitude, longitude);
    ...
    document.querySelector('.city-box').addEventListener('click', async function() {
  ...
  document.querySelector(".calender-box").addEventListener("click", function() {
    document.querySelector(".pop-up-calender-box").classList.remove("hidden-2");
  ...
  const get_next_prayer_index = function() {
    for (let i = 0; i < 8; i++) {
      if (`${new Date().getHours() > 9 ? new Date().getHours() : "0" + new Date().getHours()}:${new Date().getMinutes()}` < prayer_times[i]) {
        return i;
      }
    }
    return 0;
    ...
    if (int !== 0) {
      ...
      if (prayer_names2[int - 1] === "None") {
        ...
        document.querySelector(".next-prayer-time-to").textContent =
          ...
          (hours === 1 ? " hr " : " hrs ") +
          minutes +
          `${minutes === 1 ? " min" : " mins"} until ${int === 2 ? "Zawaal" : "Maghrib"
          }`;
      } else {
        document.querySelector(".next-prayer-name").textContent =
          prayer_names2[int - 1];
  ...
  */
  ...
  document.querySelectorAll(".calculation-choice").forEach(function(box) {
    box.addEventListener("click", function() {
      set_cookie(
        `calculation-method=${box.querySelector(".calculation-name").textContent
        };`
      );
  ...
  document
    ...
    .addEventListener("click", function() {
      document.querySelector(".intro-box").style.transform = "translate(0, -100dvh)";
      document.querySelector(".intro-box").style.opacity = '1';
      ...
      document.querySelector(".intro-done-box").addEventListener("click", function () {
        document.querySelector(".intro-box").style.transform = "translate(0, 0)";
        document.querySelector(".intro-box").style.opacity = '0';

        document.querySelector(".intro-done-box").style.pointerEvents = 'none';
        document.querySelector(".intro").style.pointerEvents = 'none';
      });
    });
  ...
  document
    ...
    .addEventListener("click", function() {
      //expanding dhikr add
      if (document.querySelector(".dhikr-nav-box").classList.contains("open-nav")) {
        document.querySelector(".dhikr-add-task-pop-up").classList.add("expand-dhikr");
      } else {
    ...
    [i].addEventListener("click", function() {
      ...
      document
        ...
        .addEventListener("click", function() {
          ...
          //resetting edits
          document.cookie = `${document.querySelectorAll(".adjust-box")[i].classList[0]
      ...
      document
        ...
        .addEventListener("click", function() {
          ...
          if (hours >= 0 && hours <= 24 && minutes >= 0 && minutes <= 59) {
            ...
            document.querySelectorAll(".adjust-time")[i].textContent = `${hours <= 9 ? "0" + hours : hours
...
const spectrumBrightnessOverlay = document.getElementById('spectrumBrightnessOverlay');
...
function updateUI() {
  ...
  rSlider.value = rgb.r; rInput.value = rgb.r;
  gSlider.value = rgb.g; gInput.value = rgb.g;
  bSlider.value = rgb.b; bInput.value = rgb.b;
  ...
  const swatchMatch = swatchesContainer.querySelector(`.swatch[data-color="${hex}"]`);
  ...
  const cellMatch = gridContainer.querySelector(`.color-cell[data-color="${hex}"]`);
  if (cellMatch) {
  ...
  if (colourToChange) {
    ...
    document.querySelector(".theme-primary-choice").style.backgroundColor = primary_color;
    document.querySelector(".theme-secondary-choice").style.backgroundColor = secondary_color;
  }
}
...
function populateGrid() {
  ...
  const grayShades = ["#ffffff", "#e5e5e5", "#bfbfbf", "#8c8c8c", "#666666", "#404040", "#1a1a1a", "#000000"];
...
//changing theme
  ...
  document.querySelectorAll(".theme-choice").forEach(function(box) {
    box.addEventListener("click", function(e) {
      //opening pop-up
      document.querySelector('.colour-picker-pop-up').classList.remove('hidden-pop-up');
...
/*get height for tabs*/
document.querySelector('.colour-picker-pop-up').style.setProperty('--tab-height', `${document.querySelector('.color-cell').offsetWidth * 6}px`)

// Close the color picker
function closeColorPicker() {
  document.querySelector('.colour-picker-pop-up').classList.add('hidden-pop-up');
}

closePickerBtn.addEventListener('click', closeColorPicker);
document.querySelector('.colour-picker-close').addEventListener('click', closeColorPicker)
...
function onRGBNumberChange() {
  ...
  rSlider.value = r; rInput.value = r;
  gSlider.value = g; gInput.value = g;
  bSlider.value = b; bInput.value = b;
...
swatchesContainer.addEventListener('click', (e) => {
  if (e.target.classList.contains('swatch') && !e.target.classList.contains('add-swatch-btn')) {
    const hex = e.target.getAttribute('data-color');
...
(function init() {
  ...
  const update_percent = function() {
    ...
    document.querySelector(".percent-progress").style.width = `${total_percent || 0
      }%`;
  };
  ...
  const add_event_listeners_dhikr = function() {
    ...
    document.querySelectorAll(".dhikr-box").forEach(function(el) {
      el.addEventListener("click", function(e) {
        if (el.querySelector(".dhikr-context").classList.contains("hidden6") && e.target.classList.contains("part-context") !== true) {
          document.querySelector('.dhikr-pop-up').style.setProperty('--dhikr-animation-start', `${el.offsetTop - document.querySelector('.dhikr-section').scrollTop}px`);
          document.querySelector(".dhikr-pop-up").classList.remove("hidden-counter");

          document.querySelector(".counter-done").textContent = el.querySelector(".dhikr-done").textContent.split("/")[0];
          document.querySelector(".counter-total").textContent = `/${el.querySelector(".dhikr-done").textContent.split("/")[1]}`;
          current_el = el;

          document.querySelector(".counter-progress").style.width = `${(el.querySelector(".dhikr-done").textContent.split("/")[0] / el.querySelector(".dhikr-done").textContent.split("/")[1]) * 100}%`;
        }
      });
    });
  };
      ...
      all_dhikrs[i].innerHTML = `<div class="dhikr-box-top">
            ...
            <div class="dhikr-box-bottom">
              ...
              <div class="dhikr-progress-info">
                <div class="dhikr-done secondary-text">${dhikr_done[i]}/${dhikr_total[i]
        }</div>
  ...
  const create_new_dhikr = function() {
    ...
    for (let i = 0; i < name.length; i++) {
      ...
      if (names[i] === name || name === ' ') {
        document.querySelector(".dhikr-name-select").style.animationPlayState = "running";

        setTimeout(function() {document.querySelector(".dhikr-name-select").style.animationPlayState = "paused"}, 450)
        return;
      } 
    } 
    document.querySelector('.dhikr-add-task-pop-up').classList.remove('expand-dhikr');
    ...
    all_dhikrs[total_dhikrs - 1].innerHTML = `<div class="dhikr-box-top">
            ...
            <div class="dhikr-box-bottom">
              ...
              <div class="dhikr-progress-info">
                <div class="dhikr-done secondary-text">${dhikr_done[total_dhikrs - 1]
      }/${dhikr_total[total_dhikrs - 1]}</div>
                <div class="dhikr-percent secondary-text">${(dhikr_done[total_dhikrs - 1] /
  ...
  document.querySelector('.dhikr-close').addEventListener('click', function() {
    if (document.querySelector('.dhikr-add-task-pop-up').classList.contains("expand-dhikr")) {
      document.querySelector('.dhikr-add-task-pop-up').classList.remove('expand-dhikr');
  ...
  document.querySelector('.dhikr-cancel').addEventListener('click', function() {
    if (document.querySelector('.dhikr-add-task-pop-up').classList.contains("expand-dhikr")) {
      document.querySelector('.dhikr-add-task-pop-up').classList.remove('expand-dhikr');
  ...
  document.querySelector('.dhikr-repeat-select-box').addEventListener('click', function() {
    document.querySelectorAll('.setting-bottom-half').forEach(function(el) {
      el.classList.add("hidden4");
    })

    document.querySelector('.setting-pop-up-box').classList.remove('hidden-pop-up');

    document.querySelector(`.dhikr-repeat-pop-up`).scrollIntoView();
    document.querySelector(`.dhikr-repeat-pop-up`).classList.remove("hidden4");
  })
  ...
  document.querySelectorAll('.dhikr-choice').forEach(function(el) {
    el.addEventListener('click', function() {
      ...
      document.querySelector('.setting-pop-up-box').classList.add('hidden-pop-up');
    })
  })
  ...
  //closing dhikr box 
  document.querySelector('.dhikr-pop-up-back-box').addEventListener('click', function() {
    current_el.querySelector('.dhikr-done').textContent = `${document.querySelector('.counter-done').textContent}${document.querySelector('.counter-total').textContent}`
    current_el.querySelector('.dhikr-progress-bar').style.width = `${document.querySelector('.counter-done').textContent / document.querySelector('.counter-total').textContent.split('/')[1] * 100}%`;
    current_el.querySelector('.dhikr-percent').textContent = `${parseInt(document.querySelector('.counter-done').textContent / document.querySelector('.counter-total').textContent.split('/')[1] * 100)}%`;
  ...
  document
    ...
    .addEventListener("click", function(e) {
      if (e.target.classList.contains("dhikr-pop-up-back")) {
        ...
        document.querySelector(".counter-progress").style.width = `${(document.querySelector(".counter-done").textContent /
          document
...
function onOrientationChange(event) {
  ...
  const directionBearing = (heading >= 180 && qiblaBearing < 180 ? qiblaBearing + 360 : qiblaBearing)
  const directionHeading = (qiblaBearing >= 180 && heading < 180 ? heading + 360 : heading)

  const turnBearing = (heading >= 180 + qiblaBearing && qiblaBearing < 180 ? qiblaBearing + 360 : qiblaBearing)
  const turnHeading = (qiblaBearing >= 180 + heading && heading < 180 ? heading + 360 : heading)

  if (directionHeading - directionBearing < 22.5 && directionHeading - directionBearing > -22.5) {
...
function createTicks() {
  ...
  for (let i = 0; i < 360; i += 6) { // Create a tick every 6 degrees
      const tick = document.createElement('div');
      tick.classList.add('tick');
      if (i % 30 === 0) {
          tick.classList.add('large'); // Larger ticks every 30 degrees
      }
      tick.style.transform = `rotate(${i}deg) translate(0, ${ -38 + (i > 180 ? (360 - i) / 180 * 3 : i / 180 * 3 )}vw)`;
